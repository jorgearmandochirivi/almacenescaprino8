...
function insert_precios($filename){
	...
	if($fp = fopen($filename,"r")){
		...
		while(!feof($fp)){
			...
			$precio = get_field( "Precio","IDPrecio","ValorVenta",$fields[1]."' AND Descuento = '0");

if( empty($precio) )
{
	$idprecio = get_maxID( "Precio","IDPrecio" );
	db_query( "INSERT INTO Precio (IDPrecio, Descripcion, ValorVenta, Publicar) VALUES ( '$idprecio','$fields[1]','$fields[1]','S' )" );
db_query( "COMMIT" );
$precio = $idprecio;
}//end if
			...
			{
				$sql_actualiza = "UPDATE Referencia SET IDPrecio = '$precio' WHERE IDReferencia = '$r_referencias->IDReferencia'";
