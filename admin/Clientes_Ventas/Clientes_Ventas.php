 ...
 //  echo  date('Y-m-d', strtotime('-15 days'));
  ...
  // if($ra->Fecha_Actualizada <= date('Y-m-d', strtotime('-15 days')))
   ...
   $queryfactura = "SELECT COUNT(ValorTotal) as Valor FROM Factura WHERE IDCliente = $ra->IDCliente GROUP BY IDCliente"; 
                                               ...
                                               $NumeroFactu = $NumeroFacturas->Valor ;
    ...
    if($r->Facturas !=$NumeroFactu &&$r->Valor_Total !=$valortot ){
    ...
    else if($r->Facturas !=$NumeroFactu && $r->Valor_Total == $valortot){
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDCiudad&in_order=".$order."&listar=".$nav->limit; %>&action=list">IDCiudad&nbsp;<% if($_GET['order_by']=="IDCiudad"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Ano&in_order=".$order."&listar=".$nav->limit; %>&action=list">Fecha Nacimiento&nbsp;<% if($_GET['order_by']=="Ano"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
	...
	&nbsp;<a href='<? echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><? 
                                                ...
                                                */
                                                ...
                                                ?></td>
						<td nowrap class=row1><? echo get_field("Ciudad","Descripcion","IDCiudad",$r->IDCiudad) ?></td>
						<td nowrap class=row1><? if( !empty( $r->Ano ) ) echo $r->Ano . "-" . $r->Mes . "-" . $r->Dia ?></td>
						<td nowrap class=row1><?php 
