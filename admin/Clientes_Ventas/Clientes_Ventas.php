...
if($permisos[0] >= 2)
{
		...
		switch (nvl($action)) {
			...
			$sql="Select C.* 
				  ...
				  and F.FechaFactura between '".$_GET[limit1]."' and '".$_GET[limit2]."' Group by F.IDCliente";
			}
			list_r($sql);
			break;
			default : 
...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<tr>
	<td>
		<table width=900 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
...
/*******************************************************************************************
		funcion Listar
*******************************************************************************************/
	function list_r_primera($sql=""){
		Global $TitleMod,$MOD,$Table,$Key,$listar;	
		
		if(empty($sql))
	 		$sql =  "SELECT * FROM $Table ORDER BY Facturas	 desc";
        
        
		        //$sql = "select c.* , f.* from Cliente c, Factura f where c.IDCliente = f.IDCliente  ";
  ...
  // $sqlinterno =  "SELECT * FROM $Table ORDER BY $Key  DESC limit 1000000";   
         ...
         $sqlinterno =  "SELECT * FROM $Table ORDER BY $Key  DESC limit 1000 ";  
    $result_count = db_query($sqlinterno);
while($ra = db_fetch_object($result_count )){

  
 //  echo  date('Y-m-d', strtotime('-15 days'));
  
   $exp_date = $ra->Fecha_Actualizada; 
   $todays_date = date("Y-m-d");
   $today = strtotime($todays_date);
   $expiration_date = strtotime($exp_date); 
  

  // if($ra->Fecha_Actualizada <= date('Y-m-d', strtotime('-15 days')))
   ...
   $queryfactura = "SELECT COUNT(ValorTotal) as Valor FROM Factura WHERE IDCliente = $ra->IDCliente GROUP BY IDCliente"; 
    ...
    $queryvalor = "SELECT SUM(ValorTotal) as Valor FROM Factura WHERE IDCliente = $ra->IDCliente GROUP BY IDCliente"; 
     ...
     // $queryupdate= "Update Cliente set Facturas = 22, Valor_Total = 22  where IDCliente = $ra->IDCliente"; 
   

    $queryupdate = null;
    
    if($ra->Facturas !=$NumeroFactu &&$ra->Valor_Total !=$valortot ){
         $queryupdate= "Update Cliente set Facturas = '$NumeroFactu', Valor_Total = '$valortot', Fecha = '$fecha', Fecha_Actualizada = NOW()  where IDCliente = $ra->IDCliente"; 
    }  
    else if($ra->Facturas !=$NumeroFactu && $ra->Valor_Total == $valortot){
         $queryupdate= "Update Cliente set Facturas = '$NumeroFactu', Fecha = '$fecha', Fecha_Actualizada = NOW()  where IDCliente = $ra->IDCliente"; 
    }
     else if($ra->Facturas ==$NumeroFactu && $ra->Valor_Total != $valortot){
        $queryupdate= "Update Cliente set Valor_Total = '$valortot', Fecha = '$fecha', Fecha_Actualizada = NOW()  where IDCliente = $ra->IDCliente"; 
    }
   ...
   // echo $query;*/
		...
		$nav->offset = 'offset';
   		...
   		(!empty($listar))? $nav->limit = $listar:$nav->limit=500;
...
?>		
<br>
<table width=900 cellpadding=0 cellspacing=0 align=center class=bordertable>
	<tr>
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Celular&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Facturas&nbsp;<?php  if($_GET['order_by']=="Celular"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Publicar&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Valor Total&nbsp;<?php  if($_GET['order_by']=="Valor Total"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
	...
	&nbsp;<a href='<?php echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><?php 
                                              /*  $query = "SELECT COUNT(ValorTotal) as Valor FROM Factura WHERE IDCliente = $r->IDCliente GROUP BY IDCliente"; 
                                                $result_count = db_query($query) or die(mysqli_error or handled by db_query);
						...
						<td nowrap class=row1><?php 
                                               /* 
                                                ...
                                                $result_valor = db_query($query) or die(mysqli_error or handled by db_query);
                                                $valor = db_fetch_object($result_valor);
                                                echo $valor->Valor ;*/
                                               echo $r->Valor_Total
                                              //  $query= "Update Cliente set Facturas = $valor->Valor where IDCliente = $r->IDCliente"; 
                                                
                                              //  db_query($query) or die(mysqli_error or handled by db_query);
                                                //echo fredy;
                                                ?></td>
...
<tr>
  <td class=texto bgcolor=#DBEAF5 colspan=10 nowrap>
  <a href="Clientes_Ventas/exporta.php?sql=<?php echo $sql; ?>&fecha1=<?php echo $_GET[limit1] ?>&fecha2=<?php echo $_GET[limit2] ?>"><img src="../images/excel_icon.gif" alt="" width="20" height="20" border="0" >Exportar Registros </a>
  </td>
</tr>
<tr>
<td class=texto bgcolor=#DBEAF5 colspan=10 nowrap>
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Cedula&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Cedula&nbsp;<?php  if($_GET['order_by']=="Cedula"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Nombre&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Nombre&nbsp;<?php  if($_GET['order_by']=="Nombre"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td><td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Apellido&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Apellidos&nbsp;<?php  if($_GET['order_by']=="Apellidos"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td><td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Telefono&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Telefono&nbsp;<?php  if($_GET['order_by']=="Telefono"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Facturas&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Facturas&nbsp;<?php  if($_GET['order_by']=="Facturas"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDCiudad&in_order=".$order."&listar=".$nav->limit; ?>&action=list">IDCiudad&nbsp;<?php  if($_GET['order_by']=="IDCiudad"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Ano&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Fecha Nacimiento&nbsp;<?php  if($_GET['order_by']=="Ano"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Valor_Total&in_order=".$order."&listar=".$nav->limit; ?>&action=list">Valor Total&nbsp;<?php  if($_GET['order_by']=="Valor_Total"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
						<td align=center  class=rowform valign=middle bgcolor=#DBEAF5 width=69>Eliminar</td>
					</tr>
<?php
//while ($row = db_fetch_object($result)) {
	...
	&nbsp;<a href='<?php echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><?php 
                                                ...
                                                */
                                                ...
                                                ?></td>
						<td nowrap class=row1><?php echo get_field("Ciudad","Descripcion","IDCiudad",$r->IDCiudad) ?></td>
						<td nowrap class=row1><?php if( !empty( $r->Ano ) ) echo $r->Ano . "-" . $r->Mes . "-" . $r->Dia ?></td>
						<td nowrap class=row1><?php 
...
?>
	<form name="frm" action="./" method="get" onsubmit="">
		<!--  return valbuscar(document.frm) -->
	<tr>
				<td class="rowform" align="center" colspan=8> 
				  Entre <input type=text readonly size=10 class=input name=limit1>
					...
					<select name="order_by" class="popup">
                        ...
                        <option value="Facturas">Facturas Realizadas</option>
                        <option value="Valor_Total">Valor_Total</option>
					</select> de forma
...
<script type="text/javascript">
   ...
   {
       var variable= document.getElementById('Buscar Por');
if(!variable.value){
