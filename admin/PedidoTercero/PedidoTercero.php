...
//envia_pedido_tercero("418","reenvio");
//echo "fin";
//exit;
		...
		switch (nvl($action)) {
			...
			case "insert" :
				...
				for($i = 1; $i < $Items; $i++){
					...
					$frm["Forro"]=$frm[$Forro];
					$frm["Plantilla"]=$frm[$Plantilla];
					$frm["Alto"]=$frm[$Alto];
					$frm["Ancho"]=$frm[$Ancho];
					$frm["Profundidad"]=$frm[$Profundidad];
					...
					if (!empty($frm[$ReferenciaCaprino])){
							$sql_inserta_detalle = "Insert into DetallePedidoTercero (IDPedidoTercero, ReferenciaCaprino, ReferenciaProveedor, CodigoColor, CueroColor, Suela, Tacon, Altura, Horma, Forro, Plantilla, Alto, Ancho, Profundidad, Producto, Precio, Observacion, IDCurvaTercero, UsuarioTrEd, FechaTrEd)
													Values('".$id."','".$frm["ReferenciaCaprino"]."','".$frm["ReferenciaProveedor"]."','".$frm["CodigoColor"]."','".$frm["CueroColor"]."','".$frm["Suela"]."','".$frm["Tacon"]."','".$frm["Altura"]."','".$frm["Horma"]."',	'".$frm["Forro"]."','".$frm["Plantilla"]."','".$frm["Alto"]."','".$frm["Ancho"]."','".$frm["Profundidad"]."','".$frm["Producto"]."', '".$frm["Precio"]."','".$frm["Observacion"]."','".$frm["IDCurvaTercero"]."','".$ID_Usuario."',NOW())";
					...
					if (!empty($frm[$ReferenciaCaprino])){
						...
						if (!empty($frm["IDDetallePedidoTercero"])){
							$sql_actualiza_detalle = "Update DetallePedidoTercero SET
													 ...
													 Horma = '".$frm["Horma"]."',
													 Forro='".$frm["Forro"]."',
													 Plantilla='".$frm["Plantilla"]."',
													 Alto='".$frm["Alto"]."',
													 Ancho='".$frm["Ancho"]."',
													 Profundidad='".$frm["Profundidad"]."',
...
<table cellpadding=1 cellspacing=0 class=bordertable align=center width="70%" >
		...
		<table width="100%" border=0 cellspacing=1 cellpadding=1 class=texto>
            ...
            <tr class=row2>
              ...
              <script language='JavaScript1.2'>
					...
					<input type="button" name="actualizar_fecha_entrega" id="actualizar_fecha_entrega" alt="<?php echo $r->IDPedidoTercero ?>"  value="Actualizar fecha entrega" class=submit>
              </td>
            </tr>
  ...
  <div id="detalle-cont">
        <table id="tabla_detalle_pedido" width="70%" border="0" align="center" style="border:1px solid #E5E5E5" cellpadding="2" cellspacing="1">
                <tbody>
                  <tr bgcolor="#9399E4" class="maintitle"  align="center" >
					...
					<td style="color:#FFFFFF; !important;">FORR</td>
					<td style="color:#FFFFFF; !important;">PLANT</td>
					<td style="color:#FFFFFF; !important;">ALT</td>
					<td style="color:#FFFFFF; !important;">ANCH</td>
					<td style="color:#FFFFFF; !important;">PROF.</td>
				  ...
				  for($i=1;$i<$detalle_inicial;$i++):  ?>
                      <tr>
                        ...
                        <td><textarea name="CueroColor<?php echo $i ?>" class="input" title="CueroColor" role="<?php echo $i ?>"  ="<?php echo $i ?>" id="CueroColor<?php echo $i ?>" cols="20" rows="3"><?php echo $array_detalle_orden[$i]["CueroColor"];  ?></textarea></td>
                        ...
                        <td><input type=text size=3 class=input   name="Tacon<?php echo $i ?>" id="Tacon<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Tacon"];  ?>"></td>
						...
						<td><input type=text size=3 class=input   name="Forro<?php echo $i ?>" id="Forro<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Forro"];  ?>"></td>
						<td><input type=text size=3 class=input   name="Plantilla<?php echo $i ?>" id="Plantilla<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Plantilla"];  ?>"></td>
						<td><input type=text size=3 class=input   name="Alto<?php echo $i ?>" id="Alto<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Alto"];  ?>"></td>
						<td><input type=text size=3 class=input   name="Ancho<?php echo $i ?>" id="Ancho<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Ancho"];  ?>"></td>
						<td><input type=text size=3 class=input   name="Profundidad<?php echo $i ?>" id="Profundidad<?php echo $i ?>" alt="<?php echo $i ?>" value="<?php echo $array_detalle_orden[$i]["Profundidad"];  ?>"></td>
                        ...
                        <select name="IDCurvaTercero<?php echo $i ?>" id="IDCurvaTercero<?php echo $i ?>" class="input" style="width:100px;">
                        	<option value=""></option>
						<?php $sql_curvas = db_query("Select * From CurvaTercero Where Publicar = 'S' Order By Nombre");
    ...
    <table align="center" width="70%">
			      ...
			      <tr>
			        <td valign="top"><table width="100%" border="0" class=texto>
			          <tbody>
			            ...
			            <tr>
			              <td>
                          <?php
                          $sql_prioridad_alta=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 1 and IDCiudad = 1 and Publicar = 'S' Order by OrdenPrioridadEntrega");
			              ...
			              <td><?php
                          $sql_prioridad_media=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 2 and IDCiudad = 1 and Publicar = 'S' Order by OrdenPrioridadEntrega");
						  while ($row_pto = db_fetch_array($sql_prioridad_media)){
							echo $row_pto['Nombre'] ."<br>";
						  }
						  ?></td>
			              <td><?php
                          $sql_prioridad_baja=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 3 and IDCiudad = 1 and Publicar = 'S' Order by OrdenPrioridadEntrega");
			        ...
			        <td valign="top"><table width="100%" border="0" class=texto>
			          <tbody>
			            ...
			            <tr>
			              <td><?php
                          $sql_prioridad_alta=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 1 and IDCiudad = 2 and Publicar = 'S' Order by OrdenPrioridadEntrega");
						  while ($row_pto = db_fetch_array($sql_prioridad_alta)){
							echo $row_pto['Nombre'] ."<br>";
						  }
						  ?></td>
			              <td><?php
                          $sql_prioridad_media=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 2 and IDCiudad = 2 and Publicar = 'S' Order by OrdenPrioridadEntrega");
						  while ($row_pto = db_fetch_array($sql_prioridad_media)){
							echo $row_pto['Nombre'] ."<br>";
						  }
						  ?></td>
			              <td><?php
                          $sql_prioridad_baja=db_query("Select Nombre from PuntoVenta Where IDTipoPrioridad = 3 and IDCiudad = 2 and Publicar = 'S' Order by OrdenPrioridadEntrega");
						  while ($row_pto = db_fetch_array($sql_prioridad_baja)){
							echo $row_pto['Nombre'] ."<br>";
						  }
						  ?></td>
			...
			<tbody>
				<tr>
				<td  align="center" bgcolor="#BDD9BF"><strong>
						Correo Visto
					</td>
					<td  align="center" bgcolor="#BDD9BF"><strong>
						IP
				...
				<?php 
					$sql_vista="SELECT * FROM PedidoTerceroVista WHERE IDPedidoTercero = '".$_GET["id"]."' ORDER BY Fecha DESC ";
					$r_vista=db_query($sql_vista);
					while($row_vista=db_fetch_array($r_vista)){
				?>
				<tr>
					<td>
						<?php echo $row_vista["Correo"]; ?>
					</td>
					<td>
					<?php echo $row_vista["IP"]; ?>
					</td>
					<td>
					<?php echo $row_vista["Fecha"]; ?>
					</td>
				</tr>
				<?php } ?>
				<tbody>
                            ...
                            <?php } ?>
						      ...
						      <tr>
								...
								if (count($array_talla)>0):
									...
									?>
							        <td class="titlemedium" nowrap align="center" width="50px"><?php echo $datos_talla['Nombre']; ?></td>
                               ...
                               <?php for($i=1;$i<=$detalle_inicial;$i++):
								...
								//Consulto solo una vez cuantos se deben pedir en la curva
								//if (!empty($array_detalle_orden[$i]["IDCurvaTercero"]) && !isset($array_total_talla_curva)){
								unset($array_total_talla_curva);
								if (!empty($array_detalle_orden[$i]["IDCurvaTercero"])){
									//echo $sql_datos_curva= "Select * From DetalleCurvaTercero Where IDCurvaTercero = '".$array_detalle_orden[$i]["IDCurvaTercero"]."'";
									$result_datos_curva = db_query($sql_datos_curva);
									while ($row_datos_curva = db_fetch_array($result_datos_curva)){
											$nombre_talla_curva = get_field("Talla","Descripcion","IDTalla",$row_datos_curva["IDTalla"]);
											$array_total_talla_curva[ $nombre_talla_curva ] += $row_datos_curva["Valor"];
											$array_ptos_curva[ $row_datos_curva["IDPuntoVenta"]] =  $row_datos_curva["IDPuntoVenta"];
									}
								}

								//print_r($array_total_talla_curva);
								//exit;
										...
										<?php
											...
											if(!empty($maximo_item[$id_talla]) ){
												...
												if(!empty($id_referencia)){
													...
													if(!isset($existencias_total_ref[$id_referencia][$nombre_talla])):
														//CONSULTO EXISTENCIAS	DE LA REF EN TODOS LOS PUNTO VENTA (solo una vez) de la curva
														if(count($array_ptos_curva)>0):
															$id_pto_curva = implode(",",$array_ptos_curva);
														else:
															$id_pto_curva = 0;
														endif;
														$sql_total_exis =  "SELECT * FROM CodificacionEspecifica CE, PuntoVentaReferencia PR
																 WHERE PR.IDReferencia = '".$id_referencia."'
																  AND IDTalla in (".$id_tallas_rel.")
																  AND PR.IDPuntoVentaReferencia = CE.IDPuntoVentaReferencia
																	AND PR.IDPuntoVentaReferencia in ($id_pto_curva)
																	";
														$query_codificacion_exis = db_query($sql_total_exis);
														$total_codificacion_exis = db_num_rows($query_codificacion_exis);
														while($rows_exis = db_fetch_array($query_codificacion_exis)):
															$existencias_total_ref[$id_referencia][$nombre_talla] += $rows_exis["Existencias"];
														endwhile;

													endif;

													//echo $existencias_total_ref[$id_referencia][$nombre_talla] .">=". $array_total_talla_curva[ $nombre_talla ];
													//print_r($existencias_total_ref);
													//exit;

													$array_maximo_a_pedir[$id_referencia][$nombre_talla] = (int)$array_total_talla_curva[ $nombre_talla ] - $existencias_total_ref[$id_referencia][$nombre_talla];



													//echo "(".(int)$array_total_talla_curva[ $nombre_talla ] ."-". $existencias_total_ref[$id_referencia][$nombre_talla].")";

													if($existencias_total_ref[$id_referencia][$nombre_talla] >= $array_total_talla_curva[ $nombre_talla ]):
															//echo "Es mayor";
															//exit;
														else:
															//echo "Es menor";
													...
													//CONSULTA EXISTENCIAS	DE LA REF EN EL PUNTO VENTA
												...
												$nombre_talla = get_field("Talla","Descripcion","IDTalla",$id_talla);
												//Como la ref no ha sido creada, le pongo un maximo grande
												$array_maximo_a_pedir[$id_referencia][$nombre_talla] = 10000;
											}
											}
											...
											//Si lo que hay en existencia es mayor a la curva lo marco como sobrante para que en otro almacen no lo pida y asi respetar la curva
											if((int)$total_existencias> (int)$maximo_item[$id_talla]=$array_datos_curva[$datos_punto_venta['IDPuntoVenta']][$datos_talla['IDTalla']] ["Maximo"]):
												$sobran = (int)$total_existencias> (int)$maximo_item[$id_talla]-$array_datos_curva[$datos_punto_venta['IDPuntoVenta']][$datos_talla['IDTalla']] ["Maximo"];
												$array_contador_sobra[$id_referencia][$nombre_talla]+=$sobran;
												echo '<br><span style="color:#EE080C !important; font-weight:bold">Sobran ('.$sobran.') </span>';
											endif;
                               ...
                               <tr>
								...
								if (count($array_talla)>0):
									...
									if(is_numeric($maximo_item[$id_talla]) ){ ?>
                                    	<?php
										if($r->IDEstadoPedidoTercero == 1):
											//Si no se ha superado el maximo a pedir segun la curva
											$nombre_talla = get_field("Talla","Descripcion","IDTalla",$datos_talla['IDTalla']);
											//echo (int)$array_pedido_talla_ref[$id_referencia][$nombre_talla] ."<". (int)$array_maximo_a_pedir[$id_referencia][$nombre_talla] . " TOT REF ". $total_codificacion_exis . "IDRef " . $id_referencia;
											//print_r($array_maximo_a_pedir);
											if((int)$array_pedido_talla_ref[$id_referencia][$nombre_talla] < (int)$array_maximo_a_pedir[$id_referencia][$nombre_talla]):
												//echo "<br>Valor pedir: " . $valor_pedir_item . "<br>";
												//echo "<br>Sobra " . $array_contador_sobra[$id_referencia][$nombre_talla] . "<br>";
												if($array_contador_sobra[$id_referencia][$nombre_talla]>0): //Esta sobrando pares
													if($valor_pedir_item<=$array_contador_sobra[$id_referencia][$nombre_talla]):
														$array_contador_sobra[$id_referencia][$nombre_talla]-=$valor_pedir_item;
														//echo "<br>Resta " . $array_contador_sobra[$id_referencia][$nombre_talla] ."-".$valor_pedir_item. "<br>";
														$valor_pedir_item = 0;
													else:
														$valor_pedir_item -= (int)$array_contador_sobra[$id_referencia][$nombre_talla];
														$array_contador_sobra[$id_referencia][$nombre_talla]=0;
													endif;
												endif;
												$array_pedido_talla_ref[$id_referencia][$nombre_talla]+=$valor_pedir_item;
											else:
												$valor_pedir_item=0;
												//if((int)$super_total_talla[$datos_talla['IDTalla']][$array_detalle_orden[$i]["IDDetallePedidoTercero"]]>0)
													//$super_total_talla[$datos_talla['IDTalla']][$array_detalle_orden[$i]["IDDetallePedidoTercero"]]--;
											endif;
											?>
			      ...
			      <tr>
			        ...
			        <td class=row1 align=center>
                    	<?php
							...
							echo $super_total_talla[$id_talla][$array_detalle_orden[$i]["IDDetallePedidoTercero"]];
...
*******************************************************************************************/
	...
	if(empty($sql))
	 	$sql =  "SELECT * FROM $Table WHERE 1 and FechaPedido >= '2018-01-01' ORDER BY $Key Desc";
	...
	$sql_punto_venta = "Select IDPuntoVenta,Nombre,IDCiudad From PuntoVenta Where Publicar = 'S'  Order By IDCiudad, Nombre";
	$result_punto_venta = db_query($sql_punto_venta);
	while ($row_punto_venta = db_fetch_array($result_punto_venta)){
		$array_punto_venta[ $row_punto_venta["IDPuntoVenta"] ] = $row_punto_venta;
	}
			...
			<td>
					<table width=100% border=0 cellspacing=1 cellpadding=0>
						...
						<td nowrap class=row1 align="center">
                         <?php
												 $qry_item_pedido =  "Select SUM(Cantidad) as TotalPedido, SUM(CantidadRecibido) as TotalRecibido From DetallePedidoTerceroReferencia Where IDPedidoTercero = '".$r->$Key."' ";
 												$sql_item_pedido = db_query($qry_item_pedido);
 										 		$row_item_pedido = db_fetch_array($sql_item_pedido);
 										 		echo $total_pedido = $row_item_pedido["TotalPedido"];
												$suma_pedido+=$total_pedido;
												?>
						...
						echo $total_recibido = $row_item_pedido["TotalRecibido"];
						$suma_recibido+=$total_recibido;
						...
						$suma_diferencia+=$diferencia_total;
						?>
                        </td>
						...
						$sql_sinfact="SELECT IDDetallePedidoTerceroReferencia FROM `DetallePedidoTerceroReferencia` WHERE `IDPedidoTercero` = ".$r->$Key." and FechaEntregaContabilidad = '' Limit 1";
						$r_sinfact=db_query($sql_sinfact);
						$total_sinfact=db_num_rows($r_sinfact);
						if((int)$total_sinfact>0){
							echo "<font style='color:#EA0C0C'> (Falta facturas)</font>";
						}
						...
						<tr >
							...
							<td></td>
							<td></td>
							<td style="font-size:12px; font-weight:bold;">Totales</td>
							<td style="font-size:12px; font-weight:bold;" align="right"><?php echo number_format($suma_pedido,0,'','.'); ?></td>
							<td style="font-size:12px; font-weight:bold;" align="right"><?php echo number_format($suma_recibido,0,'','.'); ?></td>
							<td style="font-size:12px; font-weight:bold;" align="right"><?php echo number_format($suma_diferencia,0,'','.'); ?></td>
							<td></td>
							<td></td>
						</tr>
				...
				<td class="rowform" align="center" colspan=8>
	                    ...
	                    ?>
		                    ...
		                    ?>
								...
								<tr>
									<td>Desde</td>
									<td><input  type="text" name="FechaDesde" class="input" value="<?php echo $FechaDesde?>" size="10">
									...
									<td>Hasta</td>
									<td><input  type="text" name="FechaHasta" class="input" value="<?php echo $FechaHasta?>" size="10">
