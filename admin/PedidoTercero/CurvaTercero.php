...
if($permisos[0] >= 2)
{
		...
		switch (nvl($action)) {
				...
				foreach($files AS $key => $file){
					if(!empty($file['name'])){
							...
							if(copy($file['tmp_name'], $filedir.$nombre_archivo )){
								...
								insert_curva($filedir.$nombre_archivo,$id);
								$resultado_carga = 1;
			...
			case "duplicar":
				...
				?>
				<script>
					alert("Curva duplicada con exito");
				</script>
				<?php
				list_r();
...
function print_form($id="",$newmode,$title,$submit_caption) {
	...
	$sql_punto_venta = "Select IDPuntoVenta,Nombre From PuntoVenta Where Publicar='S'  Order By IDCiudad,Nombre"; 
...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
                          ...
                          <table width="100%" border="0" cellspacing="1" cellpadding="0">
                             ...
                             <?php
                        	  if (count($array_punto_venta)>0):
							   ...
							   ?>
                                <tr>
                                    ...
                                    ?>
                                    <td bgcolor="#FFDFE0" align="center" style="font-weight:bold">
                                		<?php echo $total_fila; ?>
                                	</td>
                                  </tr>
                                 <?php endforeach;
							  ...
							  ?>
                                ...
                                <tr>
                                    <td bgcolor="#FFDFE0" align="center" style="font-weight:bold">Total</td>
                                    <?php
                                    if (count($array_talla)>0):
                                        foreach($array_talla as $id_talla => $datos_talla):
                                        ?>
                                        <td bgcolor="#FFDFE0" align="center" style="font-weight:bold"><?php  $gran_total += $suma_talla[$id_talla]; echo $suma_talla[$id_talla];  ?></td>
                                        <?php endforeach;
                                    endif;
                                    ?>
                                    <td bgcolor="#FFDFE0" align="center" style="font-weight:bold">
                                		<?php echo number_format($gran_total,"0",",","."); ?>
                                	</td>
                                  </tr>
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDCurvaTercero value="<?=$r->IDCurvaTercero ?>">    <input type=hidden name=ID value="<?php echo $r->$Key ?>">
...
*******************************************************************************************/
	...
	if($_GET["estado"]=="N")	:
		$condicion_publicar = " and Publicar = 'N' ";
	else:
		$condicion_publicar = " and Publicar = 'S' ";
	endif;
	if(empty($sql))
	 	$sql =  "SELECT * FROM $Table Where 1 ".$condicion_publicar." ORDER BY Nombre";
...
<tr>
						...
						<td align=center  class=rowform valign=middle bgcolor=#DBEAF5 width=69>Duplicar</td>
...
<tr>
	...
	&nbsp;<a href='<?php echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td align=center valign=middle nowrap width=60 class=row2><a href='<?php echo "?mod=$MOD&action=duplicar&id="; echo $r->$Key; ?>'><img src='images/continuar.gif' border='0'></a></td>
