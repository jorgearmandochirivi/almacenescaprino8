...
?>
<script>
var Check = new Array('Pregunta','Publicar');
...
<table cellpadding=1 cellspacing=0 class=bordertable align=left >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
						...
						<tr class=row2>
							<td width="50%">Pregunta
                            </td>
                            <td>
								<textarea class=input rows="10" cols="30"  name=Pregunta id=Pregunta ><?php echo $r->Pregunta ?></textarea></td>
						</tr>
						<tr class=row2>
...
<td> Poner espacio para pregunta "Cual" </td><td><?php echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Cual, 'Cual'); ?></td>
	...
	<tr class=row2>
        ...
        <td>
			<input type="text" class=input  name=Orden id=Orden value="<?php echo $r->Orden ?>" ></td>
	</tr>
						<tr class=row2>
			<td width="50%" colspan="2"></td>
			</tr>
			<tr>
			<td align=center class=row2 colspan="2"><input type=hidden name=IDFidPregunta id=IDFidPregunta value="<?php echo $r->IDFidPregunta ?>"><input type=hidden name=UsuarioTrCr value="<?php echo $r->UsuarioTrCr ?>">
...
*******************************************************************************************/
	...
	if(empty($sql))
	 	$sql =  "SELECT * FROM $Table ORDER BY Orden, $Key";
...
?>
	...
	<tr>
			<td>
				...
				<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=	&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>">Pregunta<?php  if($_GET['order_by']=="Pregunta"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
