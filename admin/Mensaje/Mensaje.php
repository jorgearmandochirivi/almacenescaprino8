...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
			<td>Mensaje</td><td><textarea name="Mensaje" rows="4" cols="40"><?php echo $r->Mensaje?></textarea></td>
			</tr>
						<tr class=row2>
							<td>Fecha Inicio</td>
							<td>
								<input type=text size=25 class=input name=FechaInicio id=Nombre value="<?php echo $r->FechaInicio ?>">
						...
						<tr class=row2>
							<td>Fecha Fin</td>
							<td>
								<input type=text size=25 class=input name=FechaFin id=Nombre value="<?php echo $r->FechaFin ?>">
								<script language="JavaScript1.2">
									<!--
										if (!document.layers)
			...
			<td> Publicar </td><td><?php echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Publicar, 'Publicar'); ?></td>
						...
						<tr class=row2>
							<td>Segunda</td>
							<td><?php echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Segunda, 'Segunda'); ?></td>
						</tr>
						<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDMensaje value="<?php echo $r->IDMensaje ?>">    <input type=hidden name=ID value="<?php echo $r->$Key ?>">
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5><a style="color: #3A4F6C;text-decoration: none" href='<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Nombre&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>'>Mensaje</a><a style="color: #3A4F6C;text-decoration: none" href='<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Nombre&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>'>&nbsp;<?php  if($_GET['order_by']=="Mensaje"){?><img src="images/<?php echo $img?>" border=0><?php }?></a></td>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5><a style="color: #3A4F6C;text-decoration: none" href='<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Codigo&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>'>Segunda</a><a style="color: #3A4F6C;text-decoration: none" href='<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Codigo&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>'>&nbsp;<?php  if($_GET['order_by']=="Segunda"){?><img src="images/<?php echo $img?>" border=0><?php }?></a></td>
...
<tr>
	...
	&nbsp;<a href='<?php echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><?php echo substr( $r->Mensaje, 0, 100 ) ?></td>
						<td nowrap class=row1><?php echo $r->Publicar ?></td>
						<td nowrap class=row1><?php echo $r->Segunda ?></td>
