...
$TitleMod ="DetalleFactura";

$Table = "DetalleFactura";
$TableJoin = "DetalleFactura";
$Key = "IDDetalleFactura";
$MOD = "DetalleFactura";
...
?>
<script>
var Check = new Array('IDDetalleFactura','IDFactura','IDReferencia','Cantidad','Publicar','UsuarioTrCr','FechaTrCr','UsuarioTrEd','FechaTrEd');
</script>
...
<tr class=row2>
			<td> IDDetalleFactura </td><td><input type=text size=25 class=input  readonly name=IDDetalleFactura id=IDDetalleFactura value="<?=$r->IDDetalleFactura ?>"> </td>
			</tr>
			<tr class=row2>
			<td> IDFactura </td><td><input type=text size=25 class=input   name=IDFactura id=IDFactura value="<?=$r->IDFactura ?>"> </td>
			</tr>
			<tr class=row2>
			<td> IDReferencia </td><td><input type=text size=25 class=input   name=IDReferencia id=IDReferencia value="<?=$r->IDReferencia ?>"> </td>
			</tr>
			<tr class=row2>
			<td> Cantidad </td><td><input type=text size=25 class=input   name=Cantidad id=Cantidad value="<?=$r->Cantidad ?>"> </td>
			</tr>
			<tr class=row2>
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=ID value="<? echo $r->$Key ?>">
...
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDDetalleFactura&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDDetalleFactura&nbsp;<% if($_GET['order_by']=="IDDetalleFactura"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDFactura&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDFactura&nbsp;<% if($_GET['order_by']=="IDFactura"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDReferencia&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDReferencia&nbsp;<% if($_GET['order_by']=="IDReferencia"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Cantidad&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">Cantidad&nbsp;<% if($_GET['order_by']=="Cantidad"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<td nowrap class=row1><? echo $r->IDDetalleFactura ?></td> <td nowrap class=row1><? echo $r->IDFactura ?></td> <td nowrap class=row1><? echo $r->IDReferencia ?></td> <td nowrap class=row1><? echo $r->Cantidad ?></td>
