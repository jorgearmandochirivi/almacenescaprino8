	...
	//start while loop to get data
		while($row = db_fetch_array($result))
		{
		  ...
		  $r_factura=db_fetch_array($sql_datos_factura);
			...
			echo get_field("EstadoGarantia","Nombre","IDEstadoGarantia",$row["IDEstadoGarantia"]) . $sep;
			...
			//REFERENCIA DEL PRODUCTO TALLA TIPO
					...
					if($row[Mayorista]=="S"){
						echo $row[ColorMayorista] . $sep;
					}
					elseif ($row[TipoRegistro]=="Reproceso"){
						echo get_field("Referencia","Numero","IDReferencia",$row[IDReferencia]) . $sep;
					...
					if($row[Mayorista]=="S"){
						...
						if ($tallap!="")
							...
							echo $tallap=get_field("Talla","Descripcion","IDTalla",get_field("CodificacionEspecifica","IDTalla","IDCodificacionEspecifica",$id_referencia_item)) . $sep;

					}

					if($row[Mayorista]=="S"){
						echo $row[TipoProductoMayorista] . $sep;
					}
					else{
						echo $tipop . $sep;
					}
			...
			echo get_field("Proveedor","Nombre","IDProveedor",$id_proveedor_ref) . $sep;
			echo $row["Remonta"] . $sep;
			echo $row["NumeroFacturaRemonta"] . $sep;
			echo $row["ValorRemonta"] . $sep;
			echo $row["PagoRemonta"] . $sep;
			echo $row["RequiereNotaCredito"] . $sep;
			echo $row["NumeroNotaCredito"] . $sep;
			echo $row["FechaNotaCreditoContabilidad"] . $sep;
			echo $row["NotaCreditoAplicada"] . $sep;
			echo $row["NumeroOrdenProduccion"] . $sep;

			echo $row["Basica"] . $sep;
			echo $row["ValorBasica"] . $sep;
			echo $row["Premium"] . $sep;
			echo $row["ValorPremium"] . $sep;
			echo $row["NumeroFacturaRestauracion"] . $sep;

			echo $row["TipoContrafuerte"] . $sep;
			echo $row["TipoCuero"] . $sep;
			echo $row["TipoPlantilla"] . $sep;
			echo $row["TipoCremallera"] . $sep;
			echo $row["TipoHerraje"] . $sep;
