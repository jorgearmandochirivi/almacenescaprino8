...
<form name="frm" action="<?php echo $PHP_SELF?>" method="post" enctype="multipart/form-data" <?php if($newmode!="delete"){?>onsubmit="setSelectOptions(document.frm.Tabla);return EvaluaReg(this,Check)"<?php }?>>
	<table width=600 border=0 cellspacing=1 cellpadding=1 class=bordertable align=center>
			<tr><td class=maintitle colspan=2>
					<?php echo $TitleMod?>
				 </td></tr>
			<tr><td class=row1 align=right>Nombre del modulo</td><td class=row2><input type=text size=25 class=tbox   name=NombreModulo value="<?php echo $r->NombreModulo ?>"> </td></tr>
			<tr>
				<td class=row1 align=right>Directorio</td>
				<td class=row2><input type=text size=25 class=tbox   name=DirectorioModulo value="<?php echo $r->DirectorioModulo ?>"></td>
			</tr>
			<?php 
			if($newmode != "insert")
			{
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden class=tbox   name=FechaTrEd value="<?php echo $r->FechaTrEd ?>"><input type=hidden class=tbox   name=UsuarioTrEd value="<?php echo $r->UsuarioTrEd ?>"><input type=hidden class=tbox   name=FechaTrCr value="<?php echo $r->FechaTrCr ?>"><input type=hidden  name=UsuarioTrCr value="<?php echo $r->UsuarioTrCr ?>"><input type=hidden name=IDModuloSite value="<?php echo $r->IDModuloSite ?>"><input type=hidden name=ID value="<?php echo $r->$Key ?>">
...
?>
	...
	<tr><td>
			...
			<tr>
				...
				<th class=titlemedium> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=NombreModulo&in_order=".$order."&listar=".$nav->limit; ?>">NombreModulo&nbsp;<?php  if($_GET['order_by']=="NombreModulo"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </th>
						<th class=titlemedium> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Publicar&in_order=".$order."&listar=".$nav->limit; ?>">Publicar&nbsp;<?php  if($_GET['order_by']=="Publicar"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </th>
