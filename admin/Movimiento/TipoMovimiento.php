...
$TitleMod ="TipoMovimiento";

$Table = "TipoMovimiento";
$TableJoin = "Movimiento";
...
?>
<script>
var Check = new Array('NombreMovimiento','Publicar');
</script>
...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=300 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
			<td>Nombre</td><td><input type=text size=25 class=input   name=NombreMovimiento id=NombreMovimiento value="<?=$r->NombreMovimiento ?>"> </td>
			</tr>
						<tr class=row2>
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDTipoMovimiento id=IDTipoMovimiento value="<?=$r->IDTipoMovimiento ?>"><input type=hidden name=UsuarioTrCr value="<?=$r->UsuarioTrCr ?>">
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5></td>
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=NombreMovimiento&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">NombreMovimiento&nbsp;<% if($_GET['order_by']=="NombreMovimiento"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<tr>
	...
	&nbsp;<a href='<? echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><? echo $r->NombreMovimiento ?></td>
