...
if($permisos[0] >= 2)
{
		switch (nvl($action)) {
			...
			if($field == "Referencia")
			{
				...
				$sql_ref = "Select PVR.IDPuntoVentaReferencia, IDCodificacionEspecifica
				...
				while($row_ref = db_fetch_array($result_ref)):
					$array_codif[]= $row_ref["IDPuntoVentaReferencia"];
				endwhile;
				...
				$sql = "SELECT M.*
				FROM Movimiento M,DetalleMovimiento DM,PuntoVenta, TipoMovimiento
				WHERE M.IDMovimiento = DM.IDMovimiento AND (DM.IDPuntoVentaReferencia in (".$id_ref."))
				AND M.IDPuntoVenta = PuntoVenta.IDPuntoVenta AND TipoMovimiento.IDTipoMovimiento = M.IDTipoMovimiento
							...
							<td class="row1" nowrap>
								<table width=100% cellspacing="1" cellpadding="1" bgcolor=#ffffff>
									<tr>
										...
										<td class=row1>
											<input type="text" class="input" name="NumeroOrden" readonly size="24" value="<?php echo $r->IDMovimiento?>">
										</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td class=row1>
											...
											<input type="text" class="input" name="FechaRemision" size="15" value="<?php echo $r->Fecha?>" readonly>
			...
			<td>
				...
				?>
					...
					<tr>
						...
						<td nowrap class=row1><?php echo $r->IDMovimiento?></td>
		...
		<tr>
			<td class="rowform" align="center" colspan=8>
				<select name="field" id="Buscar por" class="popup">
					...
					<option value="IDMovimiento">Consecutivo</option>
...
function verdetallemovimiento($id,$idpunto)
{

	Global $TitleMod,$MOD,$Table,$Key,$TableJoin;

	$sql_referencias =  "SELECT * FROM $TableJoin WHERE $Key = '$id' AND IDPuntoVenta = '$idpunto' GROUP BY IDPuntoVentaReferencia";
