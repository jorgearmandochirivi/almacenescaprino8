	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
			...
			<td> Nombre </td><td><input type=text size=25 class=input   name=Nombre id=Nombre value="<?=$r->Nombre ?>"> </td>
						...
						<tr class=row2>
			<td> Porcentaje </td><td><input type=text size=3 class=input   name=Porcentaje id=Porcentaje value="<?=$r->Porcentaje ?>" maxlength="3">%</td>
			</tr>
			<tr class=row2>
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDComision id=IDComision value="<?=$r->IDComision ?>"><input type=hidden name=UsuarioTrCr value="<?=$r->UsuarioTrCr ?>">
...
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Porcentaje&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">Porcentaje&nbsp;<% if($_GET['order_by']=="Porcentaje"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<td nowrap class=row1><? echo $r->IDComision ?></td> <td nowrap class=row1><? echo $r->Nombre ?></td> <td nowrap class=row1><? echo $r->Porcentaje."%" ?></td>
