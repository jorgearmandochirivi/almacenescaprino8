	...
	if (count($id_bonos)>0){
		foreach($id_bonos as $id_bono_value){
			...
			*/
        	...
        	$ImagenCodigo=$parametros_codigo_barras="BonoCaprino-".$valorNumericoBono.".png";
...
function crear_pdf_pedido($id_pedido_tercero){
	...
	if (!empty($id_pedido_tercero)){
		$item_detalle=1;
		$q_detalle = db_query(" SELECT * FROM DetallePedidoTercero WHERE IDPedidoTercero = '$id_pedido_tercero'");
...
<table  border="0" cellspacing="1" cellpadding="0" id=table1 width="90%" align="center">
  ...
  <?php 
												$sql_detalle = "SELECT * FROM DetallePedidoTercero WHERE IDPedidoTercero = '".$row_pedido[IDPedidoTercero]."'";
...
<table width="90%" align="center" border="1" cellpadding="0" cellspacing="0">
	<tr>
    	<td>
       	 <img src="http://www.almacenescaprino.com/images/etiquetacalzadof.jpg" width="550" height="150">
        </td>
    </tr>
</table>
                  ...
                  <tr>
                    <td valign="top"><table width="100%" border="0" >
                      <tbody>
                        ...
                        <tr>
                          <td><?php
                          $sql_prioridad_alta=db_query("Select Nombre from PuntoVenta Where Publicar='S' and IDTipoPrioridad = 1 and IDCiudad = 1");
						  while ($row_pto = db_fetch_array($sql_prioridad_alta)){
							echo $row_pto[Nombre] ."<br>";
						  }
						  ?></td>
                          <td><?php
                          $sql_prioridad_media=db_query("Select Nombre from PuntoVenta Where Publicar='S' and IDTipoPrioridad = 2 and IDCiudad = 1");
						  while ($row_pto = db_fetch_array($sql_prioridad_media)){
							echo $row_pto[Nombre] ."<br>";
						  }
						  ?></td>
                          <td><?php
                          $sql_prioridad_baja=db_query("Select Nombre from PuntoVenta Where Publicar='S' and IDTipoPrioridad = 3 and IDCiudad = 1");
                    ...
                    <table width="91%" border="0" >
                      <tbody>
                        ...
                        <tr>
                          <td><?php
                          $sql_prioridad_alta=db_query("Select Nombre from PuntoVenta Where Publicar = 'S' and IDTipoPrioridad = 1 and IDCiudad = 2");
						  while ($row_pto = db_fetch_array($sql_prioridad_alta)){
							echo $row_pto[Nombre] ."<br>";
						  }
						  ?></td>
                          <td><?php
                          $sql_prioridad_media=db_query("Select Nombre from PuntoVenta Where Publicar = 'S' and IDTipoPrioridad = 2 and IDCiudad = 2");
						  while ($row_pto = db_fetch_array($sql_prioridad_media)){
							echo $row_pto[Nombre] ."<br>";
						  }
						  ?></td>
                          <td><?php
                          $sql_prioridad_baja=db_query("Select Nombre from PuntoVenta Where Publicar='S' and IDTipoPrioridad = 3 and IDCiudad = 2");
						  while ($row_pto = db_fetch_array($sql_prioridad_baja)){
							echo $row_pto[Nombre] ."<br>";
						}
						  ?></td>
