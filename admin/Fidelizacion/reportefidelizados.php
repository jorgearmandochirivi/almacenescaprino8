<?php
	...
	$sql_cliente_mes = "Select month(FechaFactura) as Mes, count(*) as Total  From Factura where Year(FechaFactura)='".$year."' Group by month(FechaFactura)";
	...
	$sql_fidelizado ="SELECT month(FechaTrEd) as Mes, count(*) Total
					  FROM  TarjetaFidelizacion
					  WHERE YEAR( FechaTrEd ) ='".$year."'
					  Group by month(FechaTrEd)
					  ";
	...
	while($row_fidelizado = db_fetch_array($result_fidelizado)):	
		...
		//if($mes==12){
				$sql_dic = "SELECT month(FechaTrCr) as Mes, IDCliente
					  FROM  Cliente
					  WHERE YEAR( FechaTrCr ) ='".$year."' and month(FechaTrCr) = '".$mes."'					  
					  AND ClubSuavidad = 'S'";
				$result_dic = db_query($sql_dic);
				$total_dic = (int)db_num_rows($result_dic);
		//}
	...
	//FIDELIZADOS NUEVOS:
	$sql_fidelizado_new ="SELECT month(FechaTrEd) as Mes, IDCliente
					  FROM  TarjetaFidelizacion
					  WHERE YEAR( FechaTrEd ) ='".$year."'
					  UNION
					  SELECT month(FechaClubSuavidad) as Mes, IDCliente
					  FROM  Cliente
					  WHERE YEAR( FechaClubSuavidad ) ='".$year."' and month(FechaClubSuavidad) = 12					  
					  AND ClubSuavidad = 'S'
					 ";
