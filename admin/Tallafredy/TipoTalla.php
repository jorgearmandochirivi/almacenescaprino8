...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
			<td> Descripcion </td><td><textarea rows=5 cols=55 wrap=virtual name=Descripcion id=Descripcion><?php echo $r->Descripcion?></textarea></td>
			</tr>
						<tr class=row2>
			<td> Publicar </td><td><?php echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Publicar, 'Publicar'); ?></td>
			</tr>
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDTipoTalla id=IDTipoTalla value="<?php echo $r->IDTipoTalla ?>"><input type=hidden name=UsuarioTrCr value="<?php echo $r->UsuarioTrCr ?>">
			...
			?>
			<tr class=row2>
				<td colspan=2 align=right><input type=button size=10 class=submit onclick="window.open('Talla/Tallas.php?id=<?php echo $r->IDTipoTalla?>','','width=490, height=400')" value="Agregar Tallas a  <?php echo $r->Descripcion?>"></td>
			</tr>
			<tr class=row2>
				<td colspan=2>
			<?php 
...
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDTipoTalla&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>">IDTipoTalla&nbsp;<?php  if($_GET['order_by']=="IDTipoTalla"){?><img src="images/<?php echo $img?>" border=0><?php }?></a> </td>
