	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
			...
			<td width="25%">
								<div align="left">
									ValorVenta</div>
							</td><td width="25%"><input type=text size=8 class=input   name=ValorVenta id=ValorVenta value="<?=$r->ValorVenta ?>"> </td>
							<td width="25%">
									Descuento</td>
							<td width="25%"><input type=text size=3 class=input   name=Descuento id=Descuento value="<?=$r->Descuento ?>" maxlength="3">%</td>
						</tr>
						<tr class=row2>
			<td width="25%">
								<div align="left">
									...
									Publicar</div>
							</td><td width="25%"><? echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Publicar, 'Publicar'); ?></td>
							<td width="25%"></td>
							<td width="25%"></td>
						</tr>
			<tr class=row2>
			...
			<tr>
							<td align=center class=row2 colspan=4>
								<div align="left">
									<?mostrarreferencias($r->IDPrecio)?></div>
							</td>
						</tr>
			<?
			...
			<tr>
							<td colspan=3 align=center class=row2><input type=hidden name=IDPrecio id=IDPrecio value="<?=$r->IDPrecio ?>"><input type=hidden name=UsuarioTrCr value="<?=$r->UsuarioTrCr ?>">
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Descuento&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">Descuento&nbsp;<% if($_GET['order_by']=="Descuento"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<tr>
	...
	&nbsp;<a href='<? echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td align="right" nowrap class=row1><? echo number_format( $r->ValorVenta ) ?></td>
						<td align="center" nowrap class=row1><? echo $r->Descuento ?>%</td>
