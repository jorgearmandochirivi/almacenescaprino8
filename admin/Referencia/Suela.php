	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
			...
			<td colspan=2 align=center class=row2>
				...
				<input type=hidden name=action value=<?=$newmode?>><input type=hidden name=IDSuela id=IDSuela value="<?=$r->IDSuela ?>">
...
<td class=rowform nowrap bgcolor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDSuela&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDSuela&nbsp;<% if($_GET['order_by']=="IDSuela"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<td nowrap class=row1><? echo $r->IDSuela ?></td>
