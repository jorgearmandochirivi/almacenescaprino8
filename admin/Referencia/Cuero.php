...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
					<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
							<td>Nombre</td>
							<td><input type=text size=25 class=input   name=Descripcion id=Descripcion value="<?=$r->Descripcion ?>"></td>
						</tr>
						<tr class=row2>
							<td>Descripcion</td>
							<td><input type=text size=25 class=input name=DescripcionLarga id=Descripcion value="<?=$r->DescripcionLarga ?>"></td>
						</tr>
						<tr class=row2>
							<td> Publicar </td>
							<td><? echo formradiogroup(array('Si'=>'S','No'=>'N'),$r->Publicar, 'Publicar'); ?></td>
						</tr>
						<tr>
							<td colspan=2 align=center class=row2><input type=hidden name=UsuarioTrCr value="<?=$r->UsuarioTrCr ?>">
...
<input type=hidden name=IDCuero id=IDCuero value="<?=$r->IDCuero ?>"><input type=submit name=submit value="<? echo $submit_caption ?>" class=submit></td>
						</tr>
					</table>
				</td>
	</tr>
...
<td class=rowform nowrap bgColor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDCuero&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDCuero&nbsp;<% if($_GET['order_by']=="IDCuero"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
						<td class=rowform nowrap bgColor=#DBEAF5><a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Descripcion&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">Nombre <% if($_GET['order_by']=="Descripcion"){%><img src="images/<%=$img%>" border=0><%}%></a></td>
...
<td nowrap class=row1><? echo $r->IDCuero ?></td>
						<td nowrap class=row1><? echo $r->Descripcion ?></td>
						<td nowrap class=row1><? echo $r->DescripcionLarga?></td>
