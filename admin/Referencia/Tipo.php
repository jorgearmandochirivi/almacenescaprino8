...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
			...
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDTipo id=IDTipo value="<?=$r->IDTipo ?>"><input type=hidden name=UsuarioTrCr value="<?=$r->UsuarioTrCr ?>">
...
<td class=rowform nowrap bgColor=#DBEAF5> <a style="color: #3A4F6C;text-decoration: none" href="<% echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=IDTipo&in_order=".$order."&listar=".$nav->limit."&action=list"; %>">IDTipo&nbsp;<% if($_GET['order_by']=="IDTipo"){%><img src="images/<%=$img%>" border=0><%}%></a> </td>
...
<td nowrap class=row1><? echo $r->IDTipo ?></td> <td nowrap class=row1><? echo $r->Descripcion ?></td>
