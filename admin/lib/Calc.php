...
class Date_Calc
{
    ...
    function prevWeekdaytodos($day="",$month="",$year="",$format="%Y%m%d")
    {
        if(empty($year))
            $year = Date_Calc::dateNow("%Y");
        if(empty($month))
    ...
    *	$year: Numero del a–o  del actual periodo																 *
    *	Retorna	:																								 *
    *	$year_array: array con el rango de fechas dependiendo su periodicidad seleccionada						 *
    *																											 *
    *************************************************************************************************************/
    function getCalendarPeriodo($periodo,$year="",$format="%Y%m%d")
    {
        if(empty($year))
            $year = Date_Calc::dateNow("%Y");
        ...
        for($curr_month=1; $curr_month <=12; $curr_month++)
            {
            $fechas[$curr_month]=Date_Calc::beginOfMonth(sprintf("%02d",$curr_month),$year,$format);
            if(($curr_month % $periodo)==0)
            	{            	
            	$year_array[$cont] = $fechas;
            	$fechas= array();
            	$cont++;
				}
			}	
    ...
    } // end func getCalendarPeriodo
    
        
    /************************************* Funcion getActualPeriodo **********************************************
    *	Funcion para retornar el perido actual de de una fecha en una periodicidad especifica, esta funcion solo *
    *	aplica a periodicidades Bimestrales,Trimestrales y Semestrales.											 *
    *	Parametros:																								 *
    *	$periodo: Numero que hace referencia a la periodicidad que se utilizara, ej: 3 se utiliza para Trimestre *
    *	$month:	Numero del mes del actual periodo																 *
    *	$year: Numero del a–o  del actual periodo																 *
    *	Retorna	:																								 *
    *	$cont: Numero del periodo actual, dependiendo su periodicidad											 *
    *																											 *
    *************************************************************************************************************/
    
     function getActualPeriodo($periodo,$month="",$year="")
    {
        if(empty($year))
            $year = Date_Calc::dateNow("%Y");
        if(empty($month))
    ...
    } // end func getActualPeriodo
    
    
     function getCalendarQuin($month="",$year="",$format="%Y%m%d")
    {
        if(empty($year))
            $year = Date_Calc::dateNow("%Y");
        if(empty($month))
        ...
        $DiasMes=$this->daysInMonth($month);
        $periodo[1][1]=$this->dateFormat(01,$month,$year,$format);
        $periodo[1][2]=$this->dateFormat(15,$month,$year,$format);
        $periodo[2][1]=$this->dateFormat(16,$month,$year,$format);
        $periodo[2][2]=$this->dateFormat($DiasMes,$month,$year,$format);
        
        return $periodo;

    } // end func getCalendarQuin
    ...
    function getWeekdayAbbrname($day="",$month="",$year="",$length=3)
    {
        ...
        $weekday_names = Date_Calc::getWeekDaysAbbr();
        $weekday = Date_Calc::dayOfWeek($day,$month,$year);
    ...
    */
    ...
    function getWeekDaysAbbr()
    {
       ...
       // }
        ...
        $weekdays[1]="Lun";
        $weekdays[2]="Mar";
        $weekdays[3]="Mi&eacute;";
        $weekdays[4]="Jue";
        $weekdays[5]="Vie";
        $weekdays[6]="S&aacute;b";
