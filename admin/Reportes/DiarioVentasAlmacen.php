			...
			<table width='100%' align='left' border="0" cellspacing="0" cellpadding="2" class="bordertable">
				<form action="./" name="frmPuntoVenta" method="post" >
						<tr>
							...
							<td align="left" valign="middle" class="nav"> <input type="hidden" name="mod" value="DiarioVentasAlmacen"><input type="hidden" name="action" value="view"></td>
		...
		<td>
			...
			<form name="frm" action="<?=$PHP_SELF?>" method="post" onSubmit="return Evalua(document.frm)">
				...
				<?
					...
					$sql_facturas = " SELECT F.IDPuntoVenta, F.NumeroFactura,F.IDFactura, F.FechaFactura, F.ValorTotal, R.Numero, DF.ValorU,DF.PrecioU, DF.Cantidad,DF.DescuentoRef,DF.DescuentoPar, P.Descuento, F.Descuento as DescuentoFactura
										FROM Factura F, DetalleFactura DF, CodificacionEspecifica C, PuntoVentaReferencia PVR, Referencia R, Precio P
										WHERE DATE_FORMAT( F.FechaFactura,'%Y-%c-%d' ) = DATE_FORMAT('$Fecha','%Y-%c-%d' )
										...
										AND F.IDPuntoVenta = 17
										ORDER BY IDPuntoVenta,F.NumeroFactura ";

					

					$sql_facturas = " SELECT F.NumeroFactura,F.IDFactura, F.FechaFactura, F.ValorTotal, F.IDCliente, R.Numero, DF.ValorU,DF.PrecioU, DF.Cantidad,DF.DescuentoRef,DF.DescuentoPar, P.Descuento, F.Descuento as DescuentoFactura, F.ValorBono as ValorBono, PV.Nombre as NombreAlmacen,
									DF.IVA, DF.IDDetalleFactura, DF.ReteIVA, DF.ReteICA, F.IDFactura, F.IDPuntoVenta, DATE_FORMAT( F.FechaFactura,'%Y-%m-%d' ) as FechaFacturaF,PV.Nombre
									FROM Factura F, DetalleFactura DF, CodificacionEspecifica C, PuntoVentaReferencia PVR, Referencia R, Precio P, PuntoVenta PV
									WHERE 
									DATE_FORMAT( F.FechaFactura,'%Y-%c-%d' ) = DATE_FORMAT('$Fecha','%Y-%c-%d' )
									AND F.IDFactura = DF.IDFactura
									AND F.IDPuntoVenta = DF.IDPuntoVenta
									AND DF.IDCodificacionEspecifica = C.IDCodificacionEspecifica
									AND C.IDPuntoVentaReferencia = PVR.IDPuntoVentaReferencia	
									...
									AND PVR.IDReferencia = R.IDReferencia AND R.IDPrecio = P.IDPrecio AND R.Reportes <> 'N' 
									Order by PV.Nombre,NumeroFactura;";
					...
					//Puntos
					$qry_punto = db_query("SELECT * FROM PuntoVenta Where Publicar = 'S' ORDER BY Nombre");
					while($punto = db_fetch_array($qry_punto)){
						$array_puntos[$punto[IDPuntoVenta]] = $punto;
					}
					...
					{	
						foreach( $array_puntos as $idpunto => $datospuntos )
							if( $idpunto == $array_factura[IDPuntoVenta] )
							{
								$r_facturas[ $idpunto ][$i] = $array_factura;
								$i++;
							}//end if
					}//end while( $r_facturas = db_fetch_array( $qry_facturas ) )

					//echo ksort( $r_facturas );
				...
				<tr>
					...
					<tr>
				          ...
				          <td class="titlemedium" nowrap>Hora</td>
						...
						foreach( $r_facturas as $idpunto => $valorpuntos )
							foreach( $valorpuntos as $key => $valor )
						{
							//print_r( $valor );
							$class = repetition()?"row2":"row1";
							//print_r($valor);
							$puntoanterior = $punto;
							$punto = $valor[IDPuntoVenta];
							

							if( $puntoanterior <> $punto && $puntoanterior <> 0 )
							{
								$array_factura_con_comision=array();
								
							?>
						<tr>
				          <td class="titlemedium" colspan="4" align="right" nowrap>TOTALES <? echo get_field( "PuntoVenta","Nombre","IDPuntoVenta",$puntoanterior ) ?></td>
				          <td class="titlemedium" align="center" nowrap><?=$Pares ?></td>
						...
						<tr>
							<td class="maintitle" colspan="3" align="right" nowrap><? echo get_field( "PuntoVenta","Nombre","IDPuntoVenta",$punto ) ?></td>
							<td class="maintitle" align="center" nowrap></td>
						...
						<tr>
				          ...
				          <td class="<?=$class?>" align="center" nowrap><? echo substr($valor['FechaFactura'],10); ?></td>
				          ...
				          <td class="<?=$class?>" align="right" nowrap>&nbsp;
				            ...
				            <?
										...
										$TotalFactura = $valor[ValorTotal] ;
												...
												$sql_formasdepago = " SELECT * FROM FormaPagoFactura WHERE IDFactura = '$valor[IDFactura]' AND IDPuntoVenta = '$punto' ";
						...
						<!--
						<tr>
							<td class="titlemedium" colspan="4" align="right" nowrap><? echo get_field( "PuntoVenta","Nombre","IDPuntoVenta",$puntoanterior ) ?></td>
							<td class="titlemedium" align="center" nowrap><?=$Pares ?></td>
