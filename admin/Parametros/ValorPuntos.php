...
<table cellpadding=1 cellspacing=0 class=bordertable align=center >
	...
	<td>
		<table width=500 border=0 cellspacing=1 cellpadding=1 class=texto>
						<tr class=row2>
			<td>Numero de Puntos</td><td><input type="text" class="input" name="PuntosNecesarios" size="19" value="<?php echo $r->PuntosNecesarios?>" >
								
							</td>
			</tr>
			<tr class=row2>
			<td>Valor</td><td><input type="text" class="input" name="Valor" size="19" value="<?php echo $r->Valor?>" >
								
							</td>
			</tr>		
			
			<tr>
			<td colspan=2 align=center class=row2><input type=hidden name=IDValorPuntos value="<?php echo $r->IDValorPuntos ?>">    <input type=hidden name=ID value="<?php echo $r->$Key ?>">
...
<tr>
						...
						<td class=rowform nowrap bgcolor=#DBEAF5>Puntos<a style="color: #3A4F6C;text-decoration: none" href='<?php  echo "?mod=$MOD&field=".$_GET['field']."&QryString=".$_GET['QryString']."&order_by=Fecha&in_order=".$order."&listar=".$nav->limit."&action=list"; ?>'></a></td>
...
<tr>
	...
	&nbsp;<a href='<?php echo "?mod=$MOD&action=edit&id="; echo $r->$Key; ?>'><img src='images/edit.gif' border='0'></a>
						...
						<td nowrap class=row1><?php echo number_format( $r->PuntosNecesarios, 0) ?></td>
