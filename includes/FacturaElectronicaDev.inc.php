<?php
class FacturaElectronicaDev
{

  public function generar_token(){
    $curl = curl_init();  
    curl_setopt_array($curl, array(
      CURLOPT_URL => URL_FAC_ELECTRONICADEV . 'IntegrationAPI_2/api/login?username='.USER_FAC_ELECTRONICADEV.'&password='.PASS_FAC_ELECTRONICADEV,
    ...
    if(!empty($NumeroResolucion) && !empty($Codigo) && (int)$datos_cliente["IDCliente"]>0){
        ...
        if(!empty($Token)){
          ...
          //Datos Cliente
            ...
            $CorreoCliente="jorgechirivi@gmail.com";
      ...
      //FIN InvoiceTotal
        ...
        $json_factura["Documents"] = null;
          ...
          //Ejecuto llamado WS
              $curl = curl_init();
              curl_setopt_array($curl, array(
                CURLOPT_URL => URL_FAC_ELECTRONICADEV . 'integrationAPI_2/api/insertinvoice?SchemaID='.SchemaID_FAC_ELECTRONICA.'&IDNumber='.IDNumber_FAC_ELECTRONICA.'&TemplateID='.TemplateID_FAC_ELECTRONICA,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
