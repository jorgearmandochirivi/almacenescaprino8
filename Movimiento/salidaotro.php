...
if($permisos[0] >= 2)
{
		switch (nvl($action)) {
			...
			case "insert" :
				...
				echo "<script>location.href='?mod=VerMovimiento&action=add&id=$frm[IDMovimiento]';</script>";
			...
			case "update" :
				...
				//print_r($frm);
				salidamercancia($frm);
				echo "<script>location.href='?mod=Salidas';</script>";
			break;
			case "del":
...
function addCell(label){
	...
	cell.setAttribute("class","col1list"); 
	return cell;
}
function addInput(size,type,name,value,keypress, blur,cont){
	...
	if(keypress==4){
		var URL = "'Referencia/popReferencias.php?IDPuntoVenta=<?=$IDPuntoVenta?>&cont="+cont+"'";
	...
	if(blur==5)
		input.setAttribute("onblur","if(!compruebamaximo(this.value, cont)) this.value = '';"); 
...
var inp8 = addInput(5,"button","Agregar" + cont,"Agregar",4,0,cont);
...
//function selreferencia(REFERENCIA, NOMBRE, TALLA, CODIFICACION, CONT, MAXIMO){
...
var Check = new Array('IDTipoMovimiento','Numero1','Talla1','Nombre','IDCodificacion1','Cantidad1');
...
function CalculaSubtotal(  )
{
	...
	{
		elemento = "Cantidad"+i ;
		totalprod =  totalprod + ( getNum(document.frm.elements[elemento].value)*1 );
	}//end for
	
	document.frm.elements["TOTAL"].value = totalprod;
										...
										<td colspan="4">
											<table width=100% >
												...
												<tr>
													...
													<td class=col2><input type="text" class="input" name="Fecha" size="15" value='<?=fecha()." ".hora()?>' readonly> 
									...
									<td colspan="4">
										<table class="texto" width="100%" border="0" cellspacing="1" cellpadding="0" id=table1 align="center" >
											<tr>
													...
													<td align="center" class="col2list"><b>Agregar</b></td>
													<td align="center" class="col2list"><b>Referencia</b></td>
													<td align="center" class="col2list"><b>Talla</b></td>
													<td align="center" class="col2list"><b>Nombre</b></td>
													<td align="center" class="col2list"><b>Cantidad</b></td>
													<td align="center" class="col2list"><b></b></td>
												</tr>
											...
											?>
											<tr bgcolor="#dfe3e7">
													<td class="col2list" align="left"><b><?=$i?></b></td>
													<td class="col2list" align="left"><input type=button name="Agregar<?=$i?>" class=submit value=Agregar onClick="window.open('Referencia/popReferencias.php?IDPuntoVenta=<?=$IDPuntoVenta?>&cont=<?=$i?>','','width=600,height=400');"></td>
													<td class="col2list" align="left"><input type=text id="Numero<?=$i?>" rel="<?=$i ?>"   name="Numero<?=$i?>" class="input tboxReferencia" size=8></td>
													<td class="col2list" align="left"><input type=text readonly id="Talla<?=$i?>"  name="Talla<?=$i?>" class=input size=5></td>
													<td class="col2list" align="left"><input type=text readonly id="Nombre<?=$i?>"  name=Nombre<?=$i?> class=input size=15><input type=hidden name=IDCodificacion<?=$i?>></td>
											...
											<tr bgcolor="#dfe3e7">
													...
													<td class="navpic" align="left" align="center">
														<input type="text" class="input" size="5" name="TOTAL" value ="0">
													</td>
													<td class="navpic" align="left"></td>
												</tr>
