		...
		<table width=100% border=0 cellspacing=1 cellpadding=1 class="texto forumline"  >
						...
						?>
                            ...
                            <tr >
                              ...
                              <td class="col2"><? echo formpopup("Departamento","Nombre","Nombre","IDDepartamento",$r->IDDepartamento,"input\" id=\"IDDepartamento"); ?></td>
                            </tr>
                            <tr >
											...
											<td colspan="4" align=center>
												<table class="texto" width="100%" border="0" cellspacing="2" cellpadding="2" id=table1 align="center">
													...
													<tr >
														...
														<td align="left">
                                                        	<input readonly type=text  name=Numero<?=$i?> rel="<?=$i ?>" value="<? echo $frm[ $numero ] ?>" class="tbox " size=8>
                                 
                                 <?
                                 $styletarjeta = "style=\"display:none\"";
								 if( !empty( $frm[ $codigotarjeta ] ) )
								 	$styletarjeta = " ";
								 ?>                           <input type="text" name="CodigoTarjeta<?=$i?>" id="CodigoTarjeta<?=$i?>" rel="<?=$i?>" value="<? echo $frm[ $codigotarjeta ] ?>" <?=$styletarjeta ?> class="tbox " placeholder="Codigo Tarjeta" size=12 readonly />
                                                            </td>
														...
														<td align="center"><input type=text name=Cantidad<?=$i?> id=Cantidad<?=$i?> value="<? echo $frm[ $cantidad ] ?>" class="tbox" size=5 onblur="pague_2_lleve_3(); promo_segundo_par(); if(!compruebamaximo(this.value,<?=$i?>)) this.value = ''; else calculatotal(this.value,<?=$i?>);"></td>
													<td align="left"><input type=text readonly id="ValorU<?=$i?>" name=ValorU<?=$i?> value="<? echo $frm[ $valoru ] ?>" class="tbox" size=10 onblur="pague_2_lleve_3(); setvalor(this.value,<?=$i?>);calculatotal(this.value,<?=$i?>); "></td>
													<td align="center">
                                                    <input type=text name="DescuentoLin<?=$i?>" value="<? echo $frm[ $descuentolin ] ?>" onblur="calculatotal(this.value,<?=$i?>);" class="tbox" size=3 maxlength="3">
