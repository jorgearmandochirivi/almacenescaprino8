...
<?php
	$TitleMod ="CambioFactura";
	
	$Table = "CambioFactura";
	$TableJoin = "DetalleCambioFacturaEntrada";
	$Key = "IDCambioFactura";
	$MOD = "cambiar";
	$m = "Movimientos";
		$permisos = get_permiso($ID_Usuario,$m,"Cambio");
if($permisos[0] >= 2)
{
		...
		switch (nvl($action)) {
			case "insert" :
				...
				$frm['IDCambiofactura'] = insert($frm);
			...
			case "insertcliente" :
				...
				print_form($id,"insert","Realizar $TitleMod","Realizar Movimiento");
...
function selreferenciac(REFERENCIA, NOMBRE, TALLA, CODIFICACION, CONT, MAXIMO, VALORU, DESCUENTOREF){
	...
	document.frm.elements[ "Total" + CONT ].value = (getNum( document.frm.elements["ValorU" + CONT].value )*1) * (getNum( document.frm.elements["Cantidad" + CONT].value)*1 );
	formatCurrency(document.frm.elements[ "Total" + CONT ]);
...
function calculatotal(value, cont)
{
	...
	for(i=8;i<= document.frm.ITEM.value;i++){
	...
	for (var g=2; g <= 7; g++)
		if( (getNum(document.frm.elements["Total" + g].value)*1) > 0 )
		{
			TotalProductos = TotalProductos + (getNum(document.frm.elements["Total" + g].value)*1) ;
		}//end for

	Excedente = TotalSinIva - TotalProductos;
	//alert("Excedente" + Excedente);

	if( Excedente > 0 )
	{	
		Iva = Excedente - ( Excedente / ( 1 + <?=$IVA?>) ) ;
		document.frm.elements["Excedente"].value = Excedente;
...
function calculac2( i )
{
	var valoru = "ValorU" + i;
	var descuento = "DescuentoLin" + i;
	var total = "Total" + i;
	var cantidad = "Cantidad" + i;

	if( ( getNum( document.frm.elements[cantidad].value)*1 ) > 1 )
	{

		valorui = getNum(document.frm.elements[ valoru ].value ) * ( 1 - ( getNum( document.frm.elements[ descuento ].value ) / 100 ) );
		document.frm.elements[total].value = valorui * (getNum( document.frm.elements[cantidad].value)*1 );
		formatCurrency(document.frm.elements[total]);
	}//end if
}//end function
													...
													<td colspan="4">
														<table class="texto" border="0" cellspacing="1" cellpadding="0" id=table1>
															...
															<tr>
																...
																<td align="left"><input type=text name="Cantidad<?=$i ?>" class=tbox size=5 value="" onblur="calculac2( <?=$i ?> );" ></td>
																<td align="left"><input type=text readonly name="ValorU<?=$i ?>" class=tbox size=15></td>
																<td align="center"><input type=text name="DescuentoLin<?=$i ?>" value="" onblur="calculac2( <?=$i ?> );" class=tbox size=2 maxlength="2"></td>
																<td align="left"><input type=text readonly name="Total<?=$i ?>" class=tbox size=15></td>
																<td align="left"><input type=button name="Agregar<?=$i ?>" class=submit value="Referencia" onclick="window.open('Referencia/popReferenciasCambioFactura.php?IDPuntoVenta=<?=$IDPuntoVenta?>&cont=<?=$i ?>','','width=450,height=400');"></td>
																<td align="left"></td>
