...
if($permisos[0] >= 2)
{
		switch (nvl($action)) {
			...
			if( $field == 'Talla.Descripcion')
				{
					$sql = "SELECT Entrada.* FROM Entrada, Talla WHERE Talla.Descripcion LIKE '%$QryString%' AND Talla.IDTalla = Entrada.IDTalla AND IDPuntoVenta = '$IDPuntoVenta' GROUP BY IDEntrada ORDER BY IDPuntoVentaReferencia ";
				}else
				{
	...
	?>
		<table width=100% border=0 cellspacing=1 cellpadding=1 class=texto class="forumline" >
				...
				?>
					...
					<tr>
							<td align="right" style="text-align:right;" nowrap class="<?=$class?>"><? echo  $r->Remision ; ?></td>
