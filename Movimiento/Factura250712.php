...
<table class="forumline" width="500" cellspacing="1" border="0" align="center">
	...
	<td>
		<table width=100% border=0 cellspacing=1 cellpadding=1 class=texto class="forumline" >
						...
						<tr >
							<td width="40%" class="col1">C&eacute;dula ( <span class="rojo">*</span> )</td><td class=col2><input type=text size=25 class=tbox value="<?=$cedula?>" name=Cedula id=Cedula value="<?=$r->Cedula ?>"> </td>
						</tr>
						<tr >
							<td class="col1" width="40%"> Nombre ( <span class="rojo">*</span> )</td><td class="col2"><input type=text size=25 class=tbox   name=Nombre id=Nombre value="<?=$r->Nombre ?>"> </td>
						</tr>
						<tr >
							<td width="40%" class="col1"> Apellidos ( <span class="rojo">*</span> )</td><td class="col2"><input type=text size=25 class=tbox   name=Apellido id=Apellidos value="<?=$r->Apellido ?>"> </td>
						</tr>
						<tr >
							<td width="40%" class="col1"> Telefono ( <span class="rojo">*</span> )</td><td class="col2"><input type=text size=25 class=tbox   name=Telefono id=Telefono value="<?=$r->Telefono ?>"> </td>
						</tr>
						<tr >
                        ...
                        <tr >
							<td width="40%" class="col1">Numero de fidelizaci&oacute;n</td><td class="col2"><input type=text size=25 class=tbox   name=NumeroFidelizacion id=NumeroFidelizacion value="<?=$r->NumeroFidelizacion ?>"> </td>
						</tr>
						<tr >
                      ...
                      ?>
						...
						<tr >
							<td  class="col1" colspan="2">
								<table width="100%">
									<tr>
										<th>Preguntas Fidelizaci&oacute;n</th>
									</tr>
									<?
									$sql_fidelizacion = "SELECT * FROM FidPregunta WHERE Publicar = 'S' ORDER BY Orden";
									$qry_fidelizacion = db_query( $sql_fidelizacion );
									while( $r_fidelizacion = db_fetch_object( $qry_fidelizacion ) )
									{
									?>
										<tr>
											<td class="col2">
												<?=$r_fidelizacion->Pregunta ?>
											</td>
										</tr>
										<tr>
											<td class="fidOpcion">
												<ul>
									<?	
										$sql_opciones = "SELECT * FROM FidOpcion WHERE IDFidPregunta = '" . $r_fidelizacion->IDFidPregunta . "' ";
										$qry_opciones = db_query( $sql_opciones );
										while( $r_opciones = db_fetch_object( $qry_opciones ) )
										{
									?>
											<li>
												<input type="radio" name="Opcion[<?=$r_fidelizacion->IDFidPregunta ?>]" value="<?=$r_opciones->IDFidOpcion ?>">
												<label for="pregunta"><?=$r_opciones->Opcion ?></label>
											</li>
									<?
										}//end while
										
										//verificar para poner campo de cual
										if( $r_fidelizacion->Cual == 'S' )
										{
									?>
											<li>
												<textarea name="Respuesta[<?=$r_fidelizacion->IDFidPregunta ?>]" id="Respuesta[<?=$r_fidelizacion->IDFidPregunta ?>]" class="tareabox" cols="50" rows="4"></textarea>
											</li>
						...
						<tr >
							...
							<td class="col2"><input type="text" class="tbox" value="<?$r->EMail?>" name="EMail"></td>
						</tr>
						<tr >
							<td class="col1" width="40%">Preguntas Especiales</td>
							<td class="col2"><textarea name="PEspeciales" class="tareabox"><?$r->PEspeciales?></textarea></td>
						</tr>
											...
											<td colspan="4" align=center>
												<table class="texto" border="0" cellspacing="2" cellpadding="2" id=table1 align="center">
													...
													<tr >
														...
														<td align="left"><input type=button name=Agregar<?=$i?> class=submit value=Referencia onclick="window.open('Referencia/popReferencias.php?IDPuntoVenta=<?=$IDPuntoVenta?>&cont=<?=$i?>','','scrollbars = yes,width=600,height=500');"></td>
														<td align="left"><input type=text readonly name=Numero<?=$i?> value="<? echo $frm[ $numero ] ?>" class=tbox size=8></td>
														...
														<td align="center"><input type=text name=Cantidad<?=$i?> value="<? echo $frm[ $cantidad ] ?>" class=tbox size=5 onblur="if(!compruebamaximo(this.value,<?=$i?>)) this.value = ''; else calculatotal(this.value,<?=$i?>);"></td>
													<td align="left"><input type=text readonly name=ValorU<?=$i?> value="<? echo $frm[ $valoru ] ?>" class=tbox size=10></td>
