...
function print_from($IDPuntoVenta="", $Fecha=""){
 Global $dblink,$total_records,$row,$numtoshow,$Nivel,$IVA,$IDPuntoVenta,$MOD,$dirroot, $FechaHasta;
  ...
  ?>
				...
				<form action="./" name="frmPuntoVenta" method="post" name="Moviles">
						<tr>
							<td class="col1" valign="middle"><img src="admin/images/calendar_edit.png" border="0" alt=""></td>
									<td  align='left' valign='middle' class="col2">Fecha	<input  type="text" name="Fecha" class="input" value="<?=$Fecha?>">
							...
							//-->
								      ...
								      Fecha Hasta <input  type="text" name="FechaHasta" class="input" value="<?=$FechaHasta?>">
								<script language="JavaScript1.2">
							<!--
								if (!document.layers)
	...
	<table width="100%" border="0" align='center' cellspacing="1" cellpadding="0" bgcolor="#FFFFFF">
			<form name="frm" action="<?=$PHP_SELF?>" method="post" onSubmit="return Evalua(document.frm)">
				<?php
					$sql_facturas = " SELECT F.NumeroFactura,F.IDFactura, F.FechaFactura, F.ValorBono, F.ValorTotal, R.Numero, DF.ValorU,DF.PrecioU, DF.Cantidad,DF.DescuentoRef,DF.DescuentoPar, P.Descuento, F.Descuento as DescuentoFactura
										...
										AND DATE_FORMAT( F.FechaFactura,'%Y-%m-%d' ) >= DATE_FORMAT('$Fecha','%Y-%m-%d' )
