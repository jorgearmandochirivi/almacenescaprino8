	...
	?>
		...
		<table width=650 cellpadding=0 cellspacing=0 align=center class=bordertable>
			...
			if (!empty($referencia)) {
				...
				$sql_referencia = "SELECT $campo FROM Referencia WHERE $campo LIKE '%$referencia%' ORDER BY Numero ";
				$qry_referencia = db_query($sql_referencia);
				while ($r_referencia = db_fetch_object($qry_referencia)) {

					$ref = $r_referencia->$campo;

					$sql =  "SELECT * FROM $Table CE, Referencia R, PuntoVentaReferencia PR WHERE PR.IDPuntoVenta = '$puntoventa' ";
					$sql .= "AND R.$campo LIKE '$ref' AND R.IDReferencia = PR.IDReferencia ";
