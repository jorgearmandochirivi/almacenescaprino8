	...
	<table cellspacing='0' cellpadding='2' border='0' align='center' class="forumline" width="650">
		...
		<tr>
			...
			<td class="col2">
				...
				&nbsp;&nbsp;&nbsp;
				<input type=text class=tbox name=referencia>  <input type=hidden name=action value=<?=$newmode?>></td>
		</tr>
			<tr>
...
*******************************************************************************************/
	function list_r($campo="", $referencia=""){
		Global $TitleMod,$MOD,$Table,$Key,$listar,$idReferencia,$IDPuntoVenta,$Title, $Saldo;
		...
		<td>
			<table width="100%">
				...
				while( $r_referencia = db_fetch_object( $qry_referencia ) )
				{
					...
					unset($array_total_codificacion);
					unset($array_codificacion);
					...
					{
						foreach( $arraydatos as $talla => $cantidad )
						//echo "<br>Talla " . $talla . " Cantidad " . $cantidad . " Ref " . $numero_ref;
						$array_codificacion[ $ref ][ $talla  ] = array( "Numero"=>$numero_ref,"Existencia"=>$cantidad );
					}
